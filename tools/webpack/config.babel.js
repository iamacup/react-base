...
const contentVendor = require('../../src/content/bootstrap/vendorBundleList.js');

const vendor = foundationVendor.concat(contentVendor.list);

// if we are local (isDev is true) then use cheap-module-eval-source-map, otherwise use hidden-source-map
let devToolsSetting = isDev
  ? 'cheap-module-eval-source-map'
  : 'hidden-source-map';

const __API__ = JSON.stringify(process.env.API_ENDPOINT.toString());

// different source maps for different environments
if (__API__ === '1' || __API__ === '"1"') {
  devToolsSetting = 'cheap-module-source-map';
} else if (__API__ === '2' || __API__ === '"2"') {
  devToolsSetting = 'source-map';
} else if (__API__ === '3' || __API__ === '"3"') {
  devToolSetting = 'hidden-source-map';
}

console.log('Using devToolSetting: ' + devToolsSetting);
...
const getEntry = () => {
  ...
  let entry = [
    ...
    './src/foundation/client.js'
  ];

  // Prodcution
  if (!isDev) {
    entry = {
      main: './src/foundation/client.js',
      // Register vendors here
      vendor
    };
  }
